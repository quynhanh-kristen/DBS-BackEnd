...
public class BranchController {
    ...
    public String insert(Model model,
                         ...
                         @RequestParam Optional<MultipartFile> photo,
        ...
        //url
        if(branchDTO.getUrl().isEmpty()){
            if(photo == null){
                model.addAttribute("urlError", "Branch's image is required");
                valid = false;
            }
            else if (!photo.get().getOriginalFilename().endsWith(".jpg") || !photo.get().getOriginalFilename().endsWith(".png")){
                model.addAttribute("urlError", "Branch's image must be JPG or PNG file");
                valid = false;
            } else if (branchRepo.findByUrl(photo.get().getOriginalFilename()) != null) {
                model.addAttribute("urlError", "Branch's image's url is existed");
                valid = false;
            }
            else{
                branchDTO.setUrl(photo.get().getOriginalFilename());
                Path path = Paths.get("src/main/resources/static/image/branch");
                byte[] bytes = photo.get().getBytes();
                InputStream inputStream = photo.get().getInputStream();
                Path filePath = path.resolve(photo.get().getOriginalFilename());
                Files.copy(inputStream, filePath, StandardCopyOption.REPLACE_EXISTING);
            }
        }
        else{
            if(photo != null){
                if (!photo.get().getOriginalFilename().endsWith(".jpg") || !photo.get().getOriginalFilename().endsWith(".png")){
                    model.addAttribute("urlError", "Branch's image must be JPG or PNG file");
                    valid = false;
                }
                else if (branchRepo.findByUrl(photo.get().getOriginalFilename()) != null) {
                    if(photo.get().getOriginalFilename() != branchDTO.getUrl()){
                        model.addAttribute("urlError", "Branch's image's url is existed");
                        valid = false;
                    }
                }
